...
/// Decodes a hex-encoded consensus-serialized blob into a target type.
///
/// Rejects trailing bytes to prevent accepting malformed or padded data.
pub(super) fn decode_hex_blob<T: Decodable>(hex: &str) -> Result<T, JsonRpcError> {
    let bytes: Vec<u8> =
        bitcoin::hashes::hex::FromHex::from_hex(hex).map_err(|_| JsonRpcError::InvalidHex)?;
    let mut cursor = std::io::Cursor::new(&bytes);
    let value =
        T::consensus_decode(&mut cursor).map_err(|e| JsonRpcError::Decode(e.to_string()))?;
    if cursor.position() as usize != bytes.len() {
        return Err(JsonRpcError::Decode("Trailing bytes".into()));
    }
    Ok(value)
}
